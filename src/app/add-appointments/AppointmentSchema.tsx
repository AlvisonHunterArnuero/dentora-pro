...
export const appointmentSchema: yup.ObjectSchema<AppointmentFormData> = yup.object({
  ...
  user: yup.string().required("User is required").min(1, "User cannot be empty"),
  date: yup.string().required("Date is required").min(1, "Date cannot be empty"),
  startTime: yup.string().required("Start time is required").min(1, "Start time cannot be empty"),
  endTime: yup
    .string()
